...
frappe.ui.form.PrintView = class {
	...
	make() {
		...
		<div class="preview-beta-wrapper">
			<iframe width="100%" height="0" frameBorder="0" style="height: 1122.23px;"></iframe>
		</div>
		`
		);
	...
	setup_toolbar() {
		...
		this.page.set_primary_action(__("Back to Form"), () => this.go_to_form_view(), { icon: "file" });
	...
	preview() {
		this.print_wrapper.find(".print-format-skeleton").show();
		let print_format = this.get_print_format();
		const iframe = this.print_wrapper.find(".preview-beta-wrapper iframe");
		this.print_wrapper.find(".print-preview-wrapper").remove();
		...
		if (print_format.print_format_builder_beta) {
			...
			iframe.prop("src", `/api/method/frappe.utils.weasyprint.download_pdf?${params.toString()}`);
			this.print_wrapper.find(".print-format-skeleton").hide();
		}
			...
			iframe.prop("src", `/api/method/frappe.utils.print_format.download_pdf?${params.toString()}`);
			this.print_wrapper.find(".print-format-skeleton").hide();
		}
	...
	render_page(method, printit = false) {
		...
		if (method == "/api/method/frappe.utils.print_format.download_pdf?") {
			let w = window.open(
				frappe.urllib.get_full_url(
					method +
						"doctype=" +
